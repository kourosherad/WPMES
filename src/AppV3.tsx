...
import {
  Activity, Archive, ArrowDown, ArrowLeft, ArrowUp, Barcode, BellRing, Boxes, Camera, Download,
...
type RoleId = 'planning' | 'operator' | 'qc' | 'production' | 'engineering';
...
type ProjectProfile = { baseCompleted: boolean; clientName: string; orderNumber: string; itemType: 'single' | 'assembly' | null; plannedQuantity: number; dimension: string; weight: string; description: string; mainDrawingNumber: string; componentDrawings: { id: string; drawingNumber: string; description: string }[]; customFields: Record<string, string>; importedHeaders: string[]; importedRows: Record<string, string>[]; sourceFileName: string; importedRowCount: number; updatedAt?: string };
type Project = { id: string; name: string; code: string; itemType: 'single' | 'assembly' | null; status: 'DRAFT' | 'ACTIVE' | 'ON_HOLD' | 'COMPLETED'; drawings: string[]; sets: ProductionSet[]; profile?: ProjectProfile | null; releasedQuantity?: number };
type ProjectInput = { name: string; code: string };
type IssuedWorkItem = { id: string; serialNumber: string; unitNumber: number; totalQuantity: number; barcode: string; status: string; currentSet: string | null; currentStep: string | null; issuedAt: string; completedAt: string | null };
type ProductionAlert = { id: string; projectId: string; projectCode: string; projectName: string; setName: string; stepName: string; serialNumber: string; unitNumber: number; totalQuantity: number; status: string };
...
const roles = [
  { id: 'planning' as const, title: 'امور برنامه‌ریزی', action: 'تعریف پروژه و تکمیل اطلاعات پایه' },
  { id: 'operator' as const, title: 'اپراتور تولید', action: 'ثبت اتمام فرایند جاری' },
  { id: 'qc' as const, title: 'کنترل کیفیت', action: 'ثبت تأیید کنترل کیفیت' },
  { id: 'production' as const, title: 'کنترل تولید', action: 'تأیید نهایی مجموعه پس از QC' },
...
function firstPageFor(user: SessionUser): Page {
  ...
  if (user.role === 'planning' && allowed.some((item) => item.id === 'projects')) return 'projects';
...
export default function AppV3() {
  ...
  useEffect(() => {
    if (!session || !['operator', 'qc', 'production'].includes(session.role)) { setAlerts([]); return; }
    const refresh = () => apiFetch('/api/alerts').then((response) => response.ok ? response.json() : { alerts: [] }).then((data) => setAlerts(data.alerts || [])).catch(() => {});
    void refresh();
  ...
  const switchAdminRole = (nextRole: RoleId) => {
    ...
    setPage(nextRole === 'planning' || nextRole === 'engineering' || nextRole === 'qc' ? 'projects' : 'scan');
  };
  ...
  return <div className="forge-app" dir="rtl">
    ...
    <aside className={`forge-rail ${railOpen ? 'open' : ''}`}>
      ...
      <div className="rail-foot"><div className="connection"><Radio size={15} /><span><b>{session.displayName}</b><small>{role.id === 'operator' && session.accountRole !== 'admin' ? `اپراتور تولید · ${session.scope || 'بدون مجموعه'}` : role.title}</small></span></div></div>
    </aside>
    ...
    <section className="forge-workspace">
      <header className="forge-topbar">
        ...
        <div className="topbar-actions"><button className={`alert-button ${alerts.length ? 'active' : ''}`} type="button" onClick={() => navigate('scan')} title="هشدارهای در انتظار"><BellRing size={17} /><span>{alerts.length.toLocaleString('fa-IR')}</span></button><div className="live-time"><i /><span>{time.toLocaleTimeString('fa-IR', { hour: '2-digit', minute: '2-digit' })}</span></div>{session.accountRole === 'admin' && <label className="admin-role-switch"><ShieldCheck size={16} /><span><small>نقش فعال مدیر</small><select value={session.role} onChange={(event) => switchAdminRole(event.target.value as RoleId)}>{roles.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></span></label>}<div className="identity-chip"><span className="role-avatar"><UserRound size={17} /></span><span><small>{session.accountRole === 'admin' ? 'مدیر کل سامانه' : role.title}</small><b>{session.displayName}</b></span></div><button className="logout-button" type="button" onClick={() => void logout()} aria-label="خروج از سامانه"><LogOut size={17} /></button></div>
      </header>

      <main className="forge-main">
        {page === 'scan' && <ScanPage role={role} session={session} projects={projects} alerts={alerts} selectedProjectId={scanProjectId} onSelectProject={(projectId) => setScanProjectId(projectId)} onProjects={() => navigate('projects')} />}
...
function ScanPage({ role, session, projects, alerts, selectedProjectId, onSelectProject, onProjects }: { role: (typeof roles)[number]; session: SessionUser; projects: Project[]; alerts: ProductionAlert[]; selectedProjectId: string | null; onSelectProject: (projectId: string | null) => void; onProjects: () => void }) {
  ...
  const scannerMeaning = operatorStation ? `اپراتور تولید «${operatorStation}»` : role.action;
  const process = async (raw: string, inputSource: 'camera' | 'usb' | 'manual', manualReason?: string) => {
    ...
    try {
      ...
      const blockedMessage = scan?.rejectionReason === 'PRODUCTION_STATION_REQUIRED'
        ? 'برای این اپراتور هنوز مجموعه مسئول تعیین نشده است.'
        : scan?.rejectionReason === 'PRODUCTION_STATION_MISMATCH'
          ? `این QR متعلق به مجموعه «${operatorStation}» نیست.`
  ...
  return <>
    <PageHeader eyebrow="ایستگاه اسکن" title="اسکن مجموعه"><div className={`active-operation ${operatorStation ? 'station-bound' : ''}`}><span>{selectedProject ? 'پروژه فعال بارکدخوان' : operatorStation ? 'بارکدخوان اپراتور تولید' : 'عملیات این کاربر'}</span><b>{selectedProject ? selectedProject.name : scannerMeaning}</b>{selectedProject ? <button type="button" className="change-scan-project" onClick={() => { setResult(null); onSelectProject(null); onProjects(); }}>تغییر پروژه</button> : operatorStation && <small>فقط QR مرحله مرتبط با «{operatorStation}» قابل ثبت است</small>}</div></PageHeader>
    {alerts.length > 0 && <section className="production-alerts"><header><span><BellRing size={18} /><b>{alerts.length.toLocaleString('fa-IR')} اقدام در انتظار</b></span><small>{role.id === 'qc' ? 'نیازمند کنترل کیفیت' : role.id === 'production' ? 'نیازمند تأیید نهایی کنترل تولید' : 'آماده ثبت اتمام توسط اپراتور'}</small></header><div>{alerts.slice(0, 6).map((alert) => <article key={alert.id}><span>{alert.projectCode}</span><b>{alert.projectName} · {alert.setName}</b><small>محصول {alert.unitNumber.toLocaleString('fa-IR')} از {alert.totalQuantity.toLocaleString('fa-IR')} · {alert.stepName}</small></article>)}</div></section>}
    <section className="scan-console">
      <div className="scan-console-head"><div className="secure-indicator"><ShieldCheck size={17} /><span>جلسه فعال</span></div><div className={`operator-chip ${operatorStation ? 'station-chip' : ''}`}><Barcode size={16} /><span>{operatorStation ? `اپراتور ${operatorStation}` : role.title}</span></div></div>
      {!result && <div className="scan-core">
        ...
        <div className="scan-copy"><span>{selectedProject ? `کنترل کیفیت · ${selectedProject.code}` : operatorStation ? `اپراتور مجموعه ${operatorStation}` : role.id === 'production' ? 'تأیید نهایی کنترل تولید' : 'آماده اسکن'}</span><h2>کد QR را داخل قاب قرار دهید</h2></div>
        <button className={`camera-primary ${role.id === 'qc' ? 'qc-primary' : ''}`} type="button" onClick={() => setCameraOpen(true)} disabled={busy || (role.id === 'operator' && !session.scope && session.accountRole !== 'admin')}><Camera size={20} /><span>{role.id === 'qc' ? 'اسکن برای کنترل کیفیت' : operatorStation ? `اسکن در ایستگاه ${operatorStation}` : role.id === 'production' ? 'اسکن برای تأیید نهایی' : 'باز کردن بارکدخوان'}</span><ArrowLeft size={18} /></button>
...
function FlowPage({ projects, onProjects, canCreateProject }: { projects: Project[]; onProjects: () => void; canCreateProject: boolean }) {
  ...
  return <><PageHeader eyebrow="جریان تولید" title="پروژه‌های ثبت‌شده" /><section className="entity-grid">{projects.map((project) => <article className="entity-card" key={project.id}><span className="entity-icon"><FolderKanban size={22} /></span><small>{project.code}</small><h3>{project.name}</h3><p>{project.itemType === 'assembly' ? 'Assembly Part' : project.itemType === 'single' ? 'Single Part' : 'در انتظار تکمیل پروفایل مهندسی'} · {project.drawings.length.toLocaleString('fa-IR')} نقشه · {(project.sets?.length || 0).toLocaleString('fa-IR')} مجموعه</p></article>)}</section></>;
}
...
function AccessMatrixPage({ notify, projects }: { notify: (message: string) => void; projects: Project[] }) {
  ...
  const changeRole = (user: AccessUser, role: RoleId) => {
    ...
    if (role === 'planning') permissions.push('projects', 'project_create');
    ...
    patchUser(user.username, { role, scope: role === 'operator' ? user.scope : '', permissions: [...new Set(permissions)] });
  };
  ...
  const createUser = async (input: { username: string; displayName: string; password: string; role: RoleId; scope: string }) => {
    ...
    setUsers((current) => [...current, data.user]); setNewUserOpen(false); notify(input.role === 'operator' ? `اپراتور تولید «${input.scope}» با بارکدخوان اختصاصی ایجاد شد.` : 'کاربر محلی ایجاد شد؛ دسترسی‌های او را در همین صفحه تنظیم کنید.');
  };
  ...
  return <>
    ...
    {loading ? <section className="access-loading">در حال دریافت کاربران…</section> : <section className="access-matrix">
      {users.map((user) => <article className={`access-user ${user.isAdmin ? 'system-admin' : ''}`} key={user.username}>
        ...
        <div className="access-role"><label>نقش عملیاتی<select value={user.role} disabled={user.isAdmin} onChange={(event) => changeRole(user, event.target.value as RoleId)}>{user.isAdmin ? <option value="admin">مدیر کل سامانه</option> : roles.map((role) => <option key={role.id} value={role.id}>{role.title}</option>)}</select></label><span><small>مفهوم بارکدخوان</small><b>{user.isAdmin ? 'تمام عملیات‌ها' : user.role === 'operator' ? (user.scope ? `ثبت اتمام «${user.scope}»` : 'در انتظار تخصیص مجموعه') : roles.find((role) => role.id === user.role)?.action}</b></span></div>
        ...
        {!user.isAdmin && user.role === 'operator' && <label className={`operator-scope ${user.scope ? 'assigned' : 'unassigned'}`}><span><b>بارکدخوان اختصاصی اپراتور تولید</b><small>{user.scope ? `این اپراتور فقط عملیات مجموعه «${user.scope}» را ثبت می‌کند.` : 'برای فعال‌شدن اسکن، مجموعه مسئول اپراتور را انتخاب کنید.'}</small></span><select value={user.scope} onChange={(event) => patchUser(user.username, { scope: event.target.value })}><option value="">انتخاب مجموعه کاری</option>{user.scope && !productionStations.some((station) => station.key === user.scope) && <option value={user.scope}>{user.scope} (تخصیص فعلی)</option>}{productionStations.map((station) => <option key={station.key} value={station.key}>{station.title}</option>)}</select><em>{user.scope ? <><CircleCheck size={14} /> متصل به ایستگاه</> : <><CircleAlert size={14} /> بدون ایستگاه</>}</em></label>}
      </article>)}
...
function NewAccessUserModal({ productionStations, onClose, onCreate }: { productionStations: { key: string; title: string }[]; onClose: () => void; onCreate: (input: { username: string; displayName: string; password: string; role: RoleId; scope: string }) => void }) {
  const [displayName, setDisplayName] = useState(''); const [username, setUsername] = useState(''); const [password, setPassword] = useState(''); const [role, setRole] = useState<RoleId>('operator'); const [scope, setScope] = useState('');
  const stationReady = role !== 'operator' || scope.length >= 2;
  return <div className="modal-layer"><button className="modal-scrim" type="button" onClick={onClose} /><section className="forge-modal"><header><div><span>هویت محلی سامانه</span><h2>ایجاد کاربر جدید</h2></div><button type="button" onClick={onClose}><X size={19} /></button></header><div className="modal-grid"><label>نام و نام خانوادگی<input autoFocus value={displayName} onChange={(event) => setDisplayName(event.target.value)} placeholder="نام نمایشی کاربر" /></label><label>نام کاربری انگلیسی<input dir="ltr" value={username} onChange={(event) => setUsername(event.target.value.toLowerCase().replace(/[^a-z0-9._-]/g, ''))} placeholder="username" /></label></div><div className="modal-grid"><label>نقش عملیاتی<select value={role} onChange={(event) => { setRole(event.target.value as RoleId); setScope(''); }}>{roles.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label><label>رمز عبور اولیه<input dir="ltr" type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="حداقل ۱۰ کاراکتر" /></label></div>{role === 'operator' && <label className="new-user-station"><span>مجموعه مسئول اپراتور <em>الزامی</em></span><select value={scope} onChange={(event) => setScope(event.target.value)}><option value="">انتخاب مجموعه تولید</option>{productionStations.map((station) => <option key={station.key} value={station.key}>{station.title}</option>)}</select><small>این اپراتور فقط اتمام فرایندهای همین مجموعه را ثبت می‌کند.</small></label>}<div className="modal-assurance"><ShieldCheck size={18} /><span><b>{role === 'operator' && scope ? `اپراتور اختصاصی ${scope}` : 'رمز عبور به‌صورت Hash ذخیره می‌شود'}</b><small>{role === 'operator' ? 'مجموعه‌های تعریف‌شده مهندسی، حوزه مجاز اسکن اپراتور را تعیین می‌کنند.' : 'پس از ایجاد کاربر، مجوزهای دقیق او را از ماتریس همین صفحه تعیین کنید.'}</small></span></div><footer><button type="button" onClick={onClose}>انصراف</button><button className="primary" type="button" disabled={displayName.trim().length < 2 || username.length < 3 || password.length < 10 || !stationReady} onClick={() => onCreate({ displayName: displayName.trim(), username, password, role, scope })}>ایجاد کاربر</button></footer></section></div>;
}
...
function EngineeringPage({ projects, requestedProjectId, onProjects, notify, onCreateProject }: { projects: Project[]; requestedProjectId: string | null; onProjects: (projects: Project[]) => void; notify: (message: string) => void; onCreateProject: () => void }) {
  ...
  const createSet = (input: Pick<ProductionSet, 'name' | 'code'>) => {
    if (!draft) return;
    const set: ProductionSet = { id: uid(), ...input, operatorRole: input.name, kind: draft.itemType || 'assembly', steps: [{ id: uid(), name: input.name, execution: 'internal', qcRequired: true, productionControlRequired: true, barcodeAfter: false }] };
  ...
  const save = async () => {
    ...
    try {
      if (!draft.profile?.itemType || !draft.profile.mainDrawingNumber || !draft.profile.plannedQuantity) { notify('ابتدا نوع محصول، شماره نقشه و تعداد تولید را در پروفایل مهندسی ثبت کنید.'); return; }
      if (!draft.sets.length || draft.sets.some((set) => !set.steps.length)) { notify('برای ورود پروژه به تولید، حداقل یک مجموعه و یک فرایند تعریف کنید.'); return; }
      ...
      onProjects(projects.map((project) => project.id === draft.id ? data.project : project)); setDraft(structuredClone(data.project)); notify(`${Number(data.project.releasedQuantity || draft.profile.plannedQuantity).toLocaleString('fa-IR')} محصول با QR اختصاصی وارد اولین مجموعه تولید شد.`);
    } finally { setSaving(false); }
  };
  ...
  return <>
    ...
    <section className="project-engineering-shell">
      ...
      <div className="project-route-studio">{draft && <>
        <header className="project-studio-head"><div><span>{draft.code}</span><h2>{draft.name}</h2><small>{draft.itemType === 'assembly' ? 'Assembly Part' : draft.itemType === 'single' ? 'Single Part' : 'نوع محصول هنوز در پروفایل مشخص نشده'} · {draft.drawings.length.toLocaleString('fa-IR')} نقشه</small></div><button className="save-route" type="button" onClick={() => void save()} disabled={saving || !draft.itemType || draft.status === 'ACTIVE'}><Save size={17} /> {saving ? 'در حال ثبت' : draft.status === 'ACTIVE' ? 'وارد خط تولید شده' : 'ثبت نهایی و ورود به تولید'}</button></header>
        <div className="route-builder-head"><div><span>ترتیب اجرای مجموعه‌ها</span><small>کارت‌ها را Drag & Drop کنید؛ ترتیب از راست به چپ اجرا می‌شود</small></div><button type="button" onClick={() => setNewSetOpen(true)}><Plus size={16} /> افزودن مجموعه جدید</button></div>
...
function ProjectsPage({ projects, onProjects, notify, onDesignRoute, onOpenScanner, canManage, canPlan, canEngineer, isQc }: { projects: Project[]; onProjects: (projects: Project[]) => void; notify: (message: string) => void; onDesignRoute: (project: Project) => void; onOpenScanner: (project: Project) => void; canManage: boolean; canPlan: boolean; canEngineer: boolean; isQc: boolean }) {
  ...
  const [technicalProject, setTechnicalProject] = useState<Project | null>(null);
  const create = async (input: ProjectInput) => { const response = await apiFetch('/api/projects', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) }); const data = await response.json(); if (!response.ok) { notify(data.error || 'پروژه ذخیره نشد.'); return; } onProjects([...projects, data.project]); setOpen(false); notify('پروژه ثبت شد؛ تعیین نوع محصول، نقشه و تعداد تولید بر عهده امور مهندسی است.'); };
  ...
  const saveBase = async (project: Project, base: { name: string; code: string; clientName: string; orderNumber: string }) => { const response = await apiFetch(`/api/projects/${encodeURIComponent(project.id)}/profile/base`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(base) }); const data = await response.json(); if (!response.ok) { notify(data.error || 'اطلاعات پایه پروژه ذخیره نشد.'); return; } onProjects(projects.map((item) => item.id === project.id ? data.project : item)); setBaseProject(null); notify('اطلاعات پایه تکمیل شد؛ پروژه برای امور مهندسی آماده است.'); };
  const saveProfile = async (project: Project, profile: ProjectProfile) => { const response = await apiFetch(`/api/projects/${encodeURIComponent(project.id)}/profile/technical`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(profile) }); const data = await response.json(); if (!response.ok) { notify(data.error || 'اطلاعات فنی پروژه ذخیره نشد.'); return; } onProjects(projects.map((item) => item.id === project.id ? data.project : item)); setTechnicalProject(null); notify(project.status === 'ACTIVE' ? 'اطلاعات فنی و تعداد تولید به‌روزرسانی شد؛ فایل‌های QR را دوباره دریافت کنید.' : 'اطلاعات فنی پروژه ذخیره شد.'); };
  return <><PageHeader eyebrow={isQc ? 'کنترل کیفیت' : 'دفتر پروژه‌ها'} title={isQc ? 'پروژه‌های جاری' : 'پروژه‌ها'}>{canManage && <button className="heading-action" type="button" onClick={() => setOpen(true)}><Plus size={18} /> پروژه جدید</button>}</PageHeader>{projects.length ? <section className="entity-grid">{projects.map((project) => <article className={`entity-card project-card ${isQc ? 'qc-project-card' : ''}`} key={project.id}><div className="project-card-top"><span className="entity-icon"><Factory size={22} /></span><span className={`route-state ${project.status === 'ACTIVE' ? 'ready' : ''}`}>{project.status === 'ACTIVE' ? 'وارد خط تولید' : project.itemType ? 'در انتظار ثبت مسیر مهندسی' : project.profile?.baseCompleted ? 'در انتظار اطلاعات فنی' : 'در انتظار اطلاعات پایه'}</span></div><small>{project.code}</small><h3>{project.name}</h3><p>{project.itemType === 'assembly' ? 'Assembly Part' : project.itemType === 'single' ? 'Single Part' : 'نوع محصول تعیین نشده'} · {(project.profile?.plannedQuantity || 0).toLocaleString('fa-IR')} محصول · {(project.sets?.length || 0).toLocaleString('fa-IR')} مجموعه</p><div className="project-route-preview">{project.sets?.length ? project.sets.slice(0, 4).map((set, index) => <span key={set.id}><b>{(index + 1).toLocaleString('fa-IR')}</b>{set.name}</span>) : <span className="empty-preview">مسیر تولید هنوز چیده نشده است</span>}</div><div className="entity-card-actions project-actions">{isQc && <button className="open-qc-scanner" type="button" disabled={project.status !== 'ACTIVE'} onClick={() => onOpenScanner(project)}><ScanLine size={16} /> باز کردن بارکدخوان QC</button>}{canPlan && <button className="project-profile-action" type="button" disabled={project.status === 'ACTIVE'} onClick={() => setBaseProject(project)}><FileSpreadsheet size={15} /> اطلاعات پایه</button>}{canEngineer && <button className="project-profile-action" type="button" disabled={!project.profile?.baseCompleted} onClick={() => setTechnicalProject(project)}><Wrench size={15} /> اطلاعات فنی</button>}{canEngineer && <button className="barcode-register-action" type="button" disabled={project.status !== 'ACTIVE'} onClick={() => setBarcodeProject(project)}><QrCode size={15} /> شناسه‌های تولید</button>}{canEngineer && <button className="design-route" type="button" disabled={!project.itemType} onClick={() => onDesignRoute(project)}><Route size={15} /> {project.status === 'ACTIVE' ? 'مشاهده مجموعه‌ها' : 'تعریف و ترتیب مجموعه‌ها'}</button>}{canManage && <button className="danger-action" type="button" onClick={() => setDeleteProject(project)}><Trash2 size={15} /> حذف</button>}</div></article>)}</section> : <EmptyCanvas title="پروژه‌ای ثبت نشده" text="برنامه‌ریزی ابتدا نام و شماره پروژه را ثبت می‌کند." icon={<FolderKanban size={32} />} action={canManage ? <button className="empty-action" type="button" onClick={() => setOpen(true)}><Plus size={17} /> تعریف اولین پروژه</button> : undefined} />}{open && <NewProjectModal onClose={() => setOpen(false)} onCreate={(input) => void create(input)} />}{baseProject && <BaseProfileModal project={baseProject} onClose={() => setBaseProject(null)} onSave={(base) => void saveBase(baseProject, base)} />}{technicalProject && <EngineeringProfileModal project={technicalProject} onClose={() => setTechnicalProject(null)} onSave={(profile) => void saveProfile(technicalProject, profile)} />}{barcodeProject && <BarcodeRegisterModal project={barcodeProject} onClose={() => setBarcodeProject(null)} />}{deleteProject && <DeleteProjectModal project={deleteProject} onClose={() => setDeleteProject(null)} onDelete={() => void remove(deleteProject)} onArchive={() => void archive(deleteProject)} />}</>;
}

function BaseProfileModal({ project, onClose, onSave }: { project: Project; onClose: () => void; onSave: (base: { name: string; code: string; clientName: string; orderNumber: string }) => void }) {
  const [name, setName] = useState(project.name);
  const [code, setCode] = useState(project.code);
  const [clientName, setClientName] = useState(project.profile?.clientName || '');
  const [orderNumber, setOrderNumber] = useState(project.profile?.orderNumber || '');
  const valid = [name, code, clientName, orderNumber].every((value) => value.trim().length >= 2);
  return <div className="modal-layer"><button className="modal-scrim" type="button" onClick={onClose} /><section className="forge-modal staged-profile-modal"><header><div><span>مرحله ۰۱ · امور برنامه‌ریزی</span><h2>اطلاعات پایه پروژه</h2></div><button type="button" onClick={onClose}><X size={19} /></button></header><div className="profile-stage-note"><span>۱</span><div><b>تکمیل توسط امور برنامه‌ریزی</b><small>پس از ذخیره این بخش، دسترسی تکمیل اطلاعات فنی برای امور مهندسی فعال می‌شود.</small></div></div><div className="staged-profile-grid"><label>نام پروژه<input dir="auto" value={name} onChange={(event) => setName(event.target.value)} /></label><label>شماره پروژه<input dir="auto" value={code} onChange={(event) => setCode(event.target.value)} /></label><label>کارفرما<input dir="auto" value={clientName} onChange={(event) => setClientName(event.target.value)} /></label><label>شماره سفارش<input dir="auto" value={orderNumber} onChange={(event) => setOrderNumber(event.target.value)} /></label></div><footer><button type="button" onClick={onClose}>انصراف</button><button className="primary" type="button" disabled={!valid} onClick={() => onSave({ name: name.trim(), code: code.trim(), clientName: clientName.trim(), orderNumber: orderNumber.trim() })}><Save size={15} /> ذخیره و ارسال به مهندسی</button></footer></section></div>;
}

function EngineeringProfileModal({ project, onClose, onSave }: { project: Project; onClose: () => void; onSave: (profile: ProjectProfile) => void }) {
  const fallback: ProjectProfile = { baseCompleted: true, clientName: '', orderNumber: '', itemType: project.itemType, plannedQuantity: 1, dimension: '', weight: '', description: '', mainDrawingNumber: '', componentDrawings: [], customFields: {}, importedHeaders: [], importedRows: [], sourceFileName: '', importedRowCount: 0 };
  const [profile, setProfile] = useState<ProjectProfile>(() => structuredClone({ ...fallback, ...(project.profile || {}) }));
  const updateDrawing = (id: string, value: string) => setProfile((current) => ({ ...current, componentDrawings: current.componentDrawings.map((drawing) => drawing.id === id ? { ...drawing, drawingNumber: value } : drawing) }));
  const removeDrawing = (id: string) => setProfile((current) => ({ ...current, componentDrawings: current.componentDrawings.filter((drawing) => drawing.id !== id) }));
  const addDrawing = () => setProfile((current) => ({ ...current, componentDrawings: [...current.componentDrawings, { id: uid(), drawingNumber: '', description: '' }] }));
  const valid = Boolean(profile.itemType && profile.mainDrawingNumber.trim() && Number.isInteger(profile.plannedQuantity) && profile.plannedQuantity > 0 && profile.dimension.trim() && profile.weight.trim() && profile.description.trim());
  return <div className="modal-layer"><button className="modal-scrim" type="button" onClick={onClose} /><section className="forge-modal staged-profile-modal technical-profile-modal"><header><div><span>مرحله ۰۲ · امور مهندسی</span><h2>اطلاعات فنی پروژه</h2></div><button type="button" onClick={onClose}><X size={19} /></button></header><div className="profile-stage-note technical"><span>۲</span><div><b>{project.name}</b><small>{project.code} · کارفرما: {profile.clientName} · سفارش: {profile.orderNumber}</small></div></div><div className="staged-profile-grid"><label>نوع پروژه<select value={profile.itemType || ''} disabled={project.status === 'ACTIVE'} onChange={(event) => setProfile({ ...profile, itemType: event.target.value as 'single' | 'assembly' })}><option value="">انتخاب کنید</option><option value="single">Single Part</option><option value="assembly">Assembly Part</option></select></label><label>تعداد محصول<input type="number" min="1" max="10000" value={profile.plannedQuantity || ''} onChange={(event) => setProfile({ ...profile, plannedQuantity: Math.floor(Number(event.target.value)) })} /><small>{project.status === 'ACTIVE' ? 'قابل افزایش است؛ کاهش فقط برای محصولات شروع‌نشده انجام می‌شود.' : 'تعداد اولیه برنامه تولید'}</small></label><label>Dimension<input dir="auto" value={profile.dimension} onChange={(event) => setProfile({ ...profile, dimension: event.target.value })} placeholder="مثلاً 1200 × 800 × 450 mm" /></label><label>Weight<input dir="auto" value={profile.weight} onChange={(event) => setProfile({ ...profile, weight: event.target.value })} placeholder="مثلاً ۲۵۰ kg" /></label><label className="wide">Description<textarea dir="auto" rows={4} value={profile.description} onChange={(event) => setProfile({ ...profile, description: event.target.value })} placeholder="شرح فنی محصول به فارسی یا English" /></label></div><section className="drawing-editor"><header><div><b>کد نقشه و زیرنقشه‌ها</b><small>تعداد نقشه‌ها محدود به یک مورد نیست.</small></div><button type="button" onClick={addDrawing}><Plus size={15} /> افزودن زیرنقشه</button></header><label className="main-drawing">کد نقشه اصلی<input dir="auto" value={profile.mainDrawingNumber} onChange={(event) => setProfile({ ...profile, mainDrawingNumber: event.target.value })} placeholder="کد نقشه اصلی" /></label><div>{profile.componentDrawings.map((drawing, index) => <article key={drawing.id}><span>{(index + 1).toLocaleString('fa-IR')}</span><input dir="auto" value={drawing.drawingNumber} onChange={(event) => updateDrawing(drawing.id, event.target.value)} placeholder="کد زیرنقشه" /><input dir="auto" value={drawing.description} onChange={(event) => setProfile((current) => ({ ...current, componentDrawings: current.componentDrawings.map((item) => item.id === drawing.id ? { ...item, description: event.target.value } : item) }))} placeholder="شرح اختیاری" /><button type="button" onClick={() => removeDrawing(drawing.id)}><Trash2 size={14} /></button></article>)}</div></section>{project.status === 'ACTIVE' && <div className="qr-regeneration-warning"><CircleAlert size={17} /><span><b>ویرایش تعداد یا نقشه، کد QR را به‌روزرسانی می‌کند.</b><small>بعد از ذخیره، فایل‌های QR را مجدداً دریافت و لیبل‌های قبلی را جایگزین کنید.</small></span></div>}<footer><button type="button" onClick={onClose}>انصراف</button><button className="primary" type="button" disabled={!valid} onClick={() => onSave({ ...profile, componentDrawings: profile.componentDrawings.filter((drawing) => drawing.drawingNumber.trim()) })}><Save size={15} /> ذخیره اطلاعات فنی</button></footer></section></div>;
}
...
function BarcodeRegisterModal({ project, onClose }: { project: Project; onClose: () => void }) {
  ...
  return <div className="modal-layer"><button className="modal-scrim" type="button" onClick={onClose} /><section className="forge-modal barcode-register-modal"><header><div><span>{project.code}</span><h2>شناسه‌های تولید پروژه</h2></div><button type="button" onClick={onClose}><X size={19} /></button></header><div className="barcode-register-summary"><span><QrCode size={22} /></span><div><b>دفتر بارکدهای صادرشده</b><small>هر QR شامل نام پروژه، شماره نقشه و شماره محصول از تعداد کل است.</small></div><em>{items.length.toLocaleString('fa-IR')} شناسه</em></div>{loading ? <div className="barcode-register-loading">در حال دریافت شناسه‌ها…</div> : items.length ? <div className="barcode-register-list">{items.map((item) => <article key={item.id}><span className="barcode-register-icon"><QrCode size={24} /></span><div className="barcode-register-code"><small>محصول {item.unitNumber.toLocaleString('fa-IR')} از {item.totalQuantity.toLocaleString('fa-IR')}</small><b>{item.serialNumber}</b><code>{item.barcode}</code></div><div className="barcode-register-location"><small>موقعیت فعلی</small><b>{item.currentSet || 'پایان مسیر'}</b><span>{item.currentStep || statusTitle(item.status)}</span></div><span className={`barcode-status ${item.status.toLowerCase()}`}>{statusTitle(item.status)}</span><button type="button" onClick={() => void downloadSvg(item)}><Download size={15} /> SVG حک</button></article>)}</div> : <div className="barcode-register-empty"><QrCode size={32} /><b>هنوز QR صادر نشده است</b><small>پس از ثبت نهایی مسیر مهندسی، QR همه محصولات خودکار صادر می‌شود.</small></div>}<footer><button className="primary" type="button" onClick={onClose}>بستن</button></footer></section></div>;
}
...
function AssemblyProfileModal({ project, onClose, onSave, notify }: { project: Project; onClose: () => void; onSave: (profile: ProjectProfile) => void; notify: (message: string) => void }) {
  const emptyProfile: ProjectProfile = { baseCompleted: true, clientName: '', orderNumber: '', itemType: project.itemType, plannedQuantity: 1, dimension: '', weight: '', description: '', mainDrawingNumber: '', componentDrawings: [], customFields: {}, importedHeaders: [], importedRows: [], sourceFileName: '', importedRowCount: 0 };
  ...
  const titleBlockFields = ['client', 'orderNumber', 'drawingTitle', 'drawingDescription', 'scale', 'revision'];
  ...
  return <div className="modal-layer">
    ...
    <section className="forge-modal assembly-profile-modal dossier-modal">
      <header className="dossier-header">
        <div className="dossier-project-mark"><span><Boxes size={24} /></span><div><small>ENGINEERING PRODUCT PROFILE</small><h2>{project.name}</h2><p><b>{project.code}</b> · تعیین نوع، نقشه و تعداد تولید</p></div></div>
      ...
      <div className="dossier-commandbar">
        <div className="engineering-release-fields"><label>نوع محصول<select value={profile.itemType || ''} onChange={(event) => setProfile({ ...profile, itemType: event.target.value as 'single' | 'assembly' })}><option value="">انتخاب توسط مهندسی</option><option value="assembly">Assembly Part</option><option value="single">Single Part</option></select></label><label>تعداد کل تولید<input type="number" min="1" max="10000" value={profile.plannedQuantity || ''} onChange={(event) => setProfile({ ...profile, plannedQuantity: Math.max(0, Number(event.target.value)) })} /></label></div>
      ...
      <footer className="dossier-footer"><div><ShieldCheck size={17} /><span><b>ذخیره پروفایل مهندسی</b><small>هنگام ثبت نهایی مسیر، به تعداد تعیین‌شده QR ساخته و پروژه وارد اولین مجموعه می‌شود.</small></span></div><div><button type="button" onClick={onClose}>انصراف</button><button className="primary" type="button" disabled={!profile.itemType || !profile.mainDrawingNumber.trim() || profile.plannedQuantity < 1} onClick={() => onSave({ ...profile, plannedQuantity: Math.floor(profile.plannedQuantity), componentDrawings: profile.componentDrawings.filter((item) => item.drawingNumber.trim()), importedRowCount: profile.importedRows.length })}><Save size={15} /> ذخیره پروفایل مهندسی</button></div></footer>
    </section>
  </div>;
}
...
function NewProjectModal({ onClose, onCreate }: { onClose: () => void; onCreate: (value: ProjectInput) => void }) {
  const [name, setName] = useState(''); const [code, setCode] = useState('');
  return <div className="modal-layer"><button className="modal-scrim" type="button" onClick={onClose} /><section className="forge-modal new-project-modal"><header><div><span>برنامه‌ریزی تولید</span><h2>تعریف پروژه</h2></div><button type="button" onClick={onClose}><X size={19} /></button></header><div className="modal-grid"><label>نام پروژه<input autoFocus value={name} onChange={(event) => setName(event.target.value)} /></label><label>کد پروژه<input value={code} onChange={(event) => setCode(event.target.value.toUpperCase())} /></label></div><div className="assembly-create-note"><FileSpreadsheet size={20} /><span><b>نوع محصول در این مرحله تعیین نمی‌شود</b><small>امور مهندسی پس از تعریف پروژه، نوع Assembly/Single Part، شماره نقشه و تعداد تولید را در پروفایل مشخص می‌کند.</small></span></div><footer><button type="button" onClick={onClose}>انصراف</button><button className="primary" type="button" disabled={name.trim().length < 2 || code.trim().length < 2} onClick={() => onCreate({ name: name.trim(), code: code.trim() })}>ثبت پروژه و ارسال به مهندسی</button></footer></section></div>;
}
