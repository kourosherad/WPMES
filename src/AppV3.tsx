...
export default function AppV3() {
  ...
  return <div className="forge-app" dir="rtl">
    ...
    <aside className={`forge-rail ${railOpen ? 'open' : ''}`}>
      ...
      <div className="rail-foot"><div className="connection"><Radio size={15} /><span><b>{session.displayName}</b><small>{role.id === 'operator' && session.accountRole !== 'admin' ? `ایستگاه ${session.scope || 'تخصیص‌نیافته'}` : role.title}</small></span></div></div>
    </aside>
  ...
  const process = async (raw: string, inputSource: 'camera' | 'usb' | 'manual', manualReason?: string) => {
    ...
    try {
      ...
      const blockedMessage = scan?.rejectionReason === 'OPERATOR_ASSIGNMENT_REQUIRED'
        ...
        : scan?.rejectionReason === 'OPERATOR_NOT_ASSIGNED'
          ? `این QR در بارکدخوان اختصاصی «${operatorStation}» قابل تأیید نیست.`
...
function AccessMatrixPage({ notify, projects }: { notify: (message: string) => void; projects: Project[] }) {
  ...
  const createUser = async (input: { username: string; displayName: string; password: string; role: RoleId; scope: string }) => {
    ...
    setUsers((current) => [...current, data.user]); setNewUserOpen(false); notify(input.role === 'operator' ? `اپراتور «${input.scope}» با بارکدخوان اختصاصی ایجاد شد.` : 'کاربر محلی ایجاد شد؛ دسترسی‌های او را در همین صفحه تنظیم کنید.');
  };
  ...
  return <>
    ...
    {loading ? <section className="access-loading">در حال دریافت کاربران…</section> : <section className="access-matrix">
      {users.map((user) => <article className={`access-user ${user.isAdmin ? 'system-admin' : ''}`} key={user.username}>
        ...
        {!user.isAdmin && user.role === 'operator' && <label className={`operator-scope ${user.scope ? 'assigned' : 'unassigned'}`}><span><b>بارکدخوان اختصاصی اپراتور</b><small>{user.scope ? `این حساب فقط پایان مجموعه «${user.scope}» را ثبت می‌کند.` : 'برای فعال‌شدن اسکن، یکی از مجموعه‌های تعریف‌شده مهندسی را انتخاب کنید.'}</small></span><select value={user.scope} onChange={(event) => patchUser(user.username, { scope: event.target.value })}><option value="">انتخاب مجموعه مسئول</option>{user.scope && !operatorStations.some((station) => station.key === user.scope) && <option value={user.scope}>{user.scope} (تخصیص فعلی)</option>}{operatorStations.map((station) => <option key={station.key} value={station.key}>{station.title}</option>)}</select><em>{user.scope ? <><CircleCheck size={14} /> متصل به ایستگاه</> : <><CircleAlert size={14} /> بدون ایستگاه</>}</em></label>}
      </article>)}
      {!users.length && <div className="access-loading"><UsersRound size={28} /> کاربری برای تخصیص دسترسی ثبت نشده است.</div>}
    </section>}
    {newUserOpen && <NewAccessUserModal operatorStations={operatorStations} onClose={() => setNewUserOpen(false)} onCreate={(input) => void createUser(input)} />}
...
function NewAccessUserModal({ operatorStations, onClose, onCreate }: { operatorStations: { key: string; title: string }[]; onClose: () => void; onCreate: (input: { username: string; displayName: string; password: string; role: RoleId; scope: string }) => void }) {
...
function AssemblyProfileModal({ project, onClose, onSave, notify }: { project: Project; onClose: () => void; onSave: (profile: ProjectProfile) => void; notify: (message: string) => void }) {
  ...
  return <div className="modal-layer"><button className="modal-scrim" type="button" onClick={onClose} /><section className="forge-modal assembly-profile-modal"><header><div><span>Assembly Part · {project.code}</span><h2>پروفایل پروژه {project.name}</h2></div><button type="button" onClick={onClose}><X size={19} /></button></header><div className="profile-status"><span><FileSpreadsheet size={22} /></span><div><b>شناسنامه فنی پروژه مونتاژی</b><small>ساختار برای دریافت فیلدهای نهایی گزارش سربرگ آماده است.</small></div><em>نسخه اولیه</em></div><div className="profile-grid"><section className="profile-primary"><label>شماره نقشه اصلی پروژه<input value={profile.mainDrawingNumber} onChange={(event) => setProfile({ ...profile, mainDrawingNumber: event.target.value.toUpperCase() })} placeholder="شماره نقشه Assembly" /></label><div className="component-heading"><span><b>نقشه‌های تشکیل‌دهنده</b><small>جزئیات قطعات و نقشه‌های زیرمجموعه</small></span><button type="button" onClick={addDrawing}><Plus size={14} /> افزودن نقشه</button></div><div className="component-drawings">{profile.componentDrawings.map((drawing, index) => <div key={drawing.id}><span>{(index + 1).toLocaleString('fa-IR')}</span><input value={drawing.drawingNumber} onChange={(event) => updateDrawing(drawing.id, { drawingNumber: event.target.value.toUpperCase() })} placeholder="شماره نقشه" /><input value={drawing.description} onChange={(event) => updateDrawing(drawing.id, { description: event.target.value })} placeholder="شرح قطعه (اختیاری)" /><button type="button" onClick={() => removeDrawing(drawing.id)} aria-label="حذف نقشه"><Trash2 size={14} /></button></div>)}{!profile.componentDrawings.length && <div className="component-empty">هنوز نقشه‌ای ثبت نشده؛ دستی اضافه کنید یا فایل Excel وارد کنید.</div>}</div></section><aside className="excel-import-panel"><div className="excel-icon"><FileSpreadsheet size={28} /></div><h3>ورود اطلاعات اولیه از Excel</h3><p>ردیف اول به‌عنوان نام ستون‌ها خوانده می‌شود. پس از دریافت گزارش واحد، نگاشت قطعی فیلدها تکمیل خواهد شد.</p><label className="excel-picker"><input type="file" accept=".xlsx,.xls" onChange={(event) => void importExcel(event.target.files?.[0])} /><FilePlus2 size={17} /> {reading ? 'در حال خواندن فایل…' : 'انتخاب فایل Excel'}</label>{profile.sourceFileName && <div className="import-summary"><CircleCheck size={17} /><span><b>{profile.sourceFileName}</b><small>{profile.importedRows.length.toLocaleString('fa-IR')} ردیف · {profile.importedHeaders.length.toLocaleString('fa-IR')} ستون</small></span></div>}</aside></div>{profile.importedRows.length > 0 && <section className="excel-preview"><header><b>پیش‌نمایش داده‌های واردشده</b><span>{profile.importedRows.length.toLocaleString('fa-IR')} ردیف</span></header><div><table><thead><tr>{profile.importedHeaders.slice(0, 6).map((header) => <th key={header}>{header}</th>)}</tr></thead><tbody>{profile.importedRows.slice(0, 4).map((row, index) => <tr key={index}>{profile.importedHeaders.slice(0, 6).map((header) => <td key={header}>{row[header] || '—'}</td>)}</tr>)}</tbody></table></div></section>}<footer><button type="button" onClick={onClose}>انصراف</button><button className="primary" type="button" onClick={() => onSave({ ...profile, componentDrawings: profile.componentDrawings.filter((item) => item.drawingNumber.trim()), importedRowCount: profile.importedRows.length })}><Save size={15} /> ذخیره پروفایل</button></footer></section></div>;
}
